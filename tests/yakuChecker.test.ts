...
testcases.forEach(testcase => {
    describe(testcase.desc, () => {
        test(testcase.name, () => {
            const ctx = new PlayerContext({agariHai: new Hai(testcase.agariHai), isTsumo: testcase.isTsumo, playerWind: testcase.playerWind ?? TILE.WIND.EAST, roundWind: testcase.roundWind ?? TILE.WIND.EAST});
