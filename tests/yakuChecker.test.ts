...
const ctx_default: PlayerContext = new PlayerContext({agariHai: new Hai(TILE.BACK), isTsumo: false, playerWind: TILE.WIND.EAST, roundWind: TILE.WIND.EAST, kuitan: false});
...
testcases.forEach(testcase => {
    describe(testcase.desc, () => {
        test(testcase.name, () => {
            const ctx = new PlayerContext({agariHai: new Hai(testcase.agariHai), isTsumo: testcase.isTsumo, playerWind: testcase.playerWind ?? TILE.WIND.EAST, roundWind: testcase.roundWind ?? TILE.WIND.EAST, kuitan: testcase.kuitan ?? false});
